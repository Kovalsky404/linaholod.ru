...
| Сертификат linaholod.ru | **не выпущен** — DNS ещё указывает на Vercel |
| **holodlina.ru** | **работает, проверено из РФ без VPN** |

### Два домена

`holodlina.ru` и `www.holodlina.ru` уже указывают на этот сервер и **отдают
сайт** — не редиректят. Проверено с российского провайдера: 200 за 297 мс,
картинки приходят в `image/webp`.

Так сделано намеренно: пока `linaholod.ru` не переехал и не подтвердил
доступность из РФ, редирект увёл бы единственный рабочий домен на
недоступный адрес. Когда основной домен заработает — заменить в его блоке
`reverse_proxy` на `redir https://linaholod.ru{uri} permanent`, чтобы
поисковики склеили домены в один.

Оба домена ведут на ОДИН IP. Сервер различает их по имени: браузер
сообщает домен и в заголовке `Host`, и раньше того — в открытом виде при
установке шифрования (SNI). Caddy по этому имени выбирает нужный блок и
сертификат.
