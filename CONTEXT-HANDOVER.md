...
Последнее обновление: репозиторий на коммите `43821e1` (ветка `main`), рабочее дерево чистое.
...
**Тесты (всё зелёное):** `116 passed` в vitest (10 файлов) + `9 passed` в Playwright (проекты `chromium` и `mobile-chrome`).
...
### Прочее
- **Prettier ругается на 6 файлов** (`src/app/(site)/page.test.tsx`, `src/components/layout/Header.tsx`, `src/components/sections/Booking.tsx`, `src/components/sections/Booking.test.tsx`, `src/lib/seo.test.ts`, `src/sanity/content.test.ts`). Предсуществующее, к последним правкам отношения не имеет. Чинится одной командой `npm run format` — не сделано, чтобы не смешивать форматирование с содержательными правками.
