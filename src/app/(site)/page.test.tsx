...
// ───────── services доходят до селекта Booking, не только до Services ─────────
describe("F11 · services доходят до селекта Booking (не только до Services)", () => {
  it("9. опция услуги в кастомном select формы записи содержит title и price", async () => {
    mockAllData();
    const { container } = await renderHome();

    const book = container.querySelector<HTMLElement>("#book")!;
    // Select всегда рендерит <li role="option"> в DOM (видимость — CSS,
    // не conditional render) — открывать список не нужно, как в F6.
    expect(
      within(book).getByRole("option", {
        name: `${MOCK_SERVICE.title} — ${MOCK_SERVICE.price}`,
      }),
    ).toBeInTheDocument();
  });
});
...
describe("F11 · settings=null — страница не падает, секции на своих фолбэках", () => {
  it("10. рендерит страницу без ошибок и показывает дефолтные тексты/картинку секций", async () => {
    ...
    // Booking — дефолтный intro-текст.
    expect(
      within(container.querySelector<HTMLElement>("#book")!).getByText(
        /Оставьте контакт — отвечу лично/,
      ),
    ).toBeInTheDocument();
