...
describe("F15 · Reviews — ленты", () => {
  ...
  it("c2. длинный отзыв растёт вниз, а не вылезает за рамку", () => {
    // Регресс-лок: у карточки стояла жёсткая высота, и отзыв длиннее пары
    // строк выходил за её низ на ~116px, наезжая на соседей. Настоящие
    // отзывы всегда разной длины, поэтому фиксированная высота здесь —
    // не «ритм», а поломка. В jsdom высоты нет, поэтому проверяем сам
    // контракт: минимальная высота задана, фиксированная — нет.
    const { container } = render(<Reviews reviews={four} />);
